...
module.exports = async (req, res) => {
  ...
  if (resource === 'push')             return handlePush(req, res, payload);
...
/* ══════════════════════════════════════════════════════════
   PUSH — abonnements notifications Web Push
   POST   /api/user/me?resource=push   → enregistrer un abonnement
   DELETE /api/user/me?resource=push   → supprimer un abonnement
   (fusionné depuis api/user/push-subscribe.js — limite 12 fonctions Hobby)
══════════════════════════════════════════════════════════ */
async function handlePush(req, res, payload) {
  const userId = payload.userId;
  const body = await parseBody(req);
  ...
  /* ── POST : abonnement (upsert sur endpoint) ── */
  if (req.method === 'POST') {
    ...
    try {
      ...
      try {
        await supabaseRequest('PATCH',
          '/push_subscriptions?endpoint=eq.' + encodeURIComponent(endpoint),
          { user_id: userId, p256dh, auth: authKey,
